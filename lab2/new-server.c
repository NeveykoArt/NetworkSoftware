...
void *tcp_client_handler(void *arg) {
    tcp_client_args_t *client = (tcp_client_args_t *)arg;
    char buffer[1024];
    ssize_t n;

    while ((n = read(client->sockfd, buffer, sizeof(buffer) - 1)) > 0) {
        buffer[n] = '\0';
...
static int handle_tcp_connection(int tcp_sock) {
    ...
    tcp_client_args_t *args = malloc(sizeof(tcp_client_args_t));
    if (!args) {
        perror("malloc");
        close(client_fd);
        return -1;
    ...
    pthread_t tid;
    if (pthread_create(&tid, NULL, tcp_client_handler, args) != 0) {
